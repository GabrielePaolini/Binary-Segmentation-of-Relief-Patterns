...
def train(config: DictConfig) -> Trainer:
    ...
    if task_name == TASK_NAMES.TEST.value:
        config.model.neural_net_hparams.global_feat_train = False
    else:
        config.model.neural_net_hparams.global_feat_train = True
    ...
    if task_name == TASK_NAMES.FINETUNE.value:
        ...
        model = Model.load_from_checkpoint(config.model.ckpt_path, **kwargs_to_override, strict=False)
        model.criterion = hydra.utils.instantiate(config.model.criterion)   # Restore criterion (necessary if not present in model)
