...
SAMPLER = {"tiling": split_cloud_into_samples, "voronoi": split_cloud_into_GVD, "whole": sample_whole_cloud}

class HDF5Dataset(Dataset):
    """Single-file HDF5 dataset for collections of large LAS/NPY tiles."""

    def __init__(
        self,
        hdf5_file_path: str,
        epsg: str,
        data_paths_by_split_dict: DATA_PATHS_BY_SPLIT_DICT_TYPE,
        ...
        file_format: str = "las",
        sampling_method = "voronoi", #"tiling",
    ):
        """Initialization, taking care of HDF5 dataset preparation if needed, and indexation of its content.

        Args:
            data_paths_by_split_dict (Optional[DATA_PATHS_BY_SPLIT_DICT_TYPE]): should look like
                data_paths_by_split_dict = {'train': ['dir/las1.las','dir/las2.las'], 'val': [...], , 'test': [...]}
            hdf5_file_path (str): path to HDF5 dataset
            points_pre_transform (Callable): Function to turn pdal points into a pyg Data object.
            tile_width (Number, optional): width of a LAS/NPY tile. Defaults to 1000.
        ...
        if not data_paths_by_split_dict:
            log.warning(
                "No data_paths_by_split_dict given, pre-computed HDF5 dataset is therefore used."
            )
            return

        # Add data for all LAS/NPY Files into a single hdf5 file.
        create_hdf5(
    ...
    def _get_data(self, sample_hdf5_path: str) -> Data:
        ...
        return Data(
            ...
            normals=torch.from_numpy(grp["normals"][...]),
...
def create_hdf5(
    ...
    """Create a HDF5 dataset file from las/npy.

    Args:
        data_paths_by_split_dict ([DATA_PATHS_BY_SPLIT_DICT_TYPE]): should look like
                data_paths_by_split_dict = {'train': ['dir/las1.las','dir/las2.las'], 'val': [...], , 'test': [...]},
        hdf5_file_path (str): path to HDF5 dataset,
        epsg (str): epsg to force the reading with
        tile_width (Number, optional): width of a LAS/NPY tile. 1000 by default,
    ...
    for split, data_paths in data_paths_by_split_dict.items():
        with h5py.File(hdf5_file_path, "a") as f:
            if split not in f:
                f.create_group(split)
        for data_path in tqdm(data_paths, desc=f"Preparing {split} set..."):
            basename = os.path.basename(data_path)

            # Delete dataset for incomplete LAS/NPY entry, to start from scratch.
            ...
            with h5py.File(hdf5_file_path, "a") as hdf5_file:
                ...
                for sample_number, (sample_idx, sample_points) in enumerate(
                    ...
                    tile_size_series.append(sample_idx.shape[0])
                    ...
                    # Aggiunte normali
                    hdf5_file.create_dataset(
                        os.path.join(hdf5_path, "normals"),
                        data["normals"].shape,
                        dtype="f",
                        data=data["normals"],
                    )
    ...
    '''
    # DEBUG PLOT HISTOGRAM OF TILE SIZES
    import pandas as pd
    import numpy as np
    import matplotlib.pyplot as plt
    tile_size = pd.Series(np.array(tile_size_series))
    tile_size.plot.hist(grid=True, bins=40, rwidth=0.9,
                   color='#607c8e')
    plt.title('Tile sizes for whole dataset')
    plt.xlabel('Counts')
    plt.ylabel('Number of vertices per tile')
    plt.show()
    '''
