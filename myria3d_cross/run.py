...
from myria3d.pctl.dataset.utils import get_las_paths_by_split_dict as get_data_paths_by_split_dicts
TASK_NAME_DETECTION_STRING = "task.task_name="
DEFAULT_DIRECTORY = "configs/"
...
def launch_predict(config: DictConfig):
    ...
    src_data_iterable = glob(config.predict.src_data)    # Previously src_las
    for config.predict.src_data in tqdm(src_data_iterable):
        predict(config)


@hydra.main(config_path="configs/", config_name="config.yaml", version_base="1.1")
def launch_hdf5(config: DictConfig):
    """Build an HDF5 file from a directory with las/npy files."""

    # Pretty print config using Rich library
    if config.get("print_config"):
        utils.print_config(config, resolve=False)

    data_paths_by_split_dict = get_data_paths_by_split_dicts(
        config.datamodule.get("data_dir"), config.datamodule.get("split_csv_path")
    )
    create_hdf5(
        data_paths_by_split_dict=data_paths_by_split_dict,
        ...
        file_format=config.datamodule.format,
    )
